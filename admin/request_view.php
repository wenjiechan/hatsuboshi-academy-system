...
$request_success = $_SESSION['admin_request_success'] ?? '';
$request_error = $_SESSION['admin_request_error'] ?? '';
unset($_SESSION['admin_request_success'], $_SESSION['admin_request_error']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $_SESSION['admin_request_success'] = producer_request_handle_action(
            $pdo,
            $request,
            (int) $_SESSION['id'],
            'admin',
        ...
        if ($request_action === 'manual_edit') {
            header('Location: ' . producer_request_manual_edit_url($request, 'admin'));
            exit;
        }
    } catch (Throwable $exception) {
        $_SESSION['admin_request_error'] = $exception->getMessage();
    }

    header('Location: /gakumas-sms/admin/request_view.php?id=' . (int) $request['id']);
    exit;
}
